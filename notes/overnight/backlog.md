...
## 【人工输入登记 2026-09-18 01:30】EDGE_CASCADE 架构文档合入 main（非裁定）

- `origin/main` 新增 `docs/EDGE_CASCADE.md`（端侧 MT-LNN 级联 + 云端 LLM + 本仓库
  RAG 作 grounding 层；架构设想、未原型验证）。已按分支纪律 merge 进 overnight/loop
  （`6aac411`，无冲突），pytest 363 passed / refs PASS。
- **证据政策冲突（待人工修正）**：该文档把"金融文档 QA 适配器（91.7%）"标为
  "✅ 已实测（回归基线）"，但 `docs/RESULTS.md` 负面声明第 2 条已裁定
  "91.7% 无本仓库产物支撑，证据链补齐前不得引用"。循环不擅自改动 Owner 文档，
  请 AricRedemption 修正该行或授权循环补测量。
- **与决策包的关联**：EDGE_CASCADE 的云端层质量声明（无论 91.7% 真伪）恰恰需要
  选项 1 的真实基准测量来支撑/证伪——级联架构的 grounding 层若引用无证据数字，
  选项 1 的优先级进一步上升。
