...
## P4 — 下一候选方向（待下轮定级）：predictive world model 筛查 [IDEA]

- P3 表中次选：用下一句/掩码预测分数做 passage 重排信号，纯本地可跑。
- **但**：R2/R3 阴性提示"同规模合成任务无区分度"——若继续合成路线，需升级任务
  难度（多跳/组合结构）或模型规模（d≥192、2 层 trunk）；否则建议先做 R1 的
  人工裁定事项解阻塞（48 题真实标注集），让类脑模块在真实基准上受测。
- 下轮动作：先做方向定级（升级合成 vs 解阻塞真实基准的取舍——后者涉及已登记的
  人工判定项），再决定预注册内容。
