...
赛事官方地址：[https://tianchi.aliyun.com/competition/entrance/532486](https://tianchi.aliyun.com/competition/entrance/532486)
...
![](https://cdn.nlark.com/yuque/0/2026/png/69403934/1781683355905-ffd809bc-7e41-4b31-9c59-62a0dde9c604.png)
...
1. 访问赛事页面：[https://tianchi.aliyun.com/competition/entrance/532486](https://tianchi.aliyun.com/competition/entrance/532486)

1. 在「赛题与数据」栏目下载 `public_dataset_a.zip`（约 274MB）

1. 将压缩包放到项目根目录的 `data/` 下
...
1. 调用 `pymupdf4llm` 把 86 份 PDF 逐页转成 Markdown，输出到 `data/processed_pymupdf4llm/`
...
![](https://cdn.nlark.com/yuque/0/2026/png/69403934/1781683356318-b43ca47d-430b-4641-9b98-e9a09b6c6788.png)

1. 登录阿里云百炼控制台：[https://bailian.console.aliyun.com/](https://bailian.console.aliyun.com/)

1. 进入左侧导航栏 **API Key** 页面（或直接访问 [https://bailian.console.aliyun.com/?tab=model#/api-key）](https://bailian.console.aliyun.com/?tab=model#/api-key）)

1. 单击 **创建 API Key**，选择默认业务空间，权限选 **全部**

1. 创建成功后立即复制并保存完整的 Key（关闭弹窗后无法再次查看明文）

![](https://cdn.nlark.com/yuque/0/2026/png/69403934/1781683356118-c65e3bf4-f104-4e4a-acc8-efd8466100e7.png)
...
- 深度思考模型用法（含 `enable_thinking` 参数说明）：[https://help.aliyun.com/zh/model-studio/deep-thinking](https://help.aliyun.com/zh/model-studio/deep-thinking)

- DashScope API 参考：[https://help.aliyun.com/zh/model-studio/qwen-api-via-dashscope](https://help.aliyun.com/zh/model-studio/qwen-api-via-dashscope)
...
1. **金融知识本身门槛高**。保险条款中的"等待期""免赔额""责任免除"，监管法规中的"受益所有人识别""客户尽职调查"，这些概念对非专业人士来说理解成本很高。模型如果没有足够的金融领域预训练知识，光凭 prompt 很难做到精准理解。

1. **优化方案往往有副作用**。比如引入检索可以提升长文档覆盖率，但可能漏掉关键否定词；让模型做详细推理可以提升判断准确性，但会大幅增加 Token 消耗。每改一个地方，可能解决一个问题，又引入另一个问题。
...
1. **在线查询**：答题时先判断题目是否涉及结构化比较，如果是则优先查知识图谱；如果是需要原文引用的条款解释题，再回退到原始文本检索
...
[3] 阿里云百炼 DashScope API 参考。参见 [https://help.aliyun.com/zh/model-studio/](https://help.aliyun.com/zh/model-studio/)
