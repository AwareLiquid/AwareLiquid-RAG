...
1. **如何在文档中精确定位关键信息**（计算题要找到公式，条款题要找到对应章节）
...
1. **精确条款匹配**：每个选项的推理都依赖具体条款编号（第四十七条、第八十二条），模型必须定位到精确条款而非泛泛而谈

1. **条件判断**：选项 A 中的"资产负债率超过 70%"是一个明确的数值阈值，模型必须准确提取并比较

1. **逻辑否定**：选项 B 和 D 都是"看起来合理但实际错误"的干扰项，模型需要识别出"变更募集资金用途是普通决议""修改章程是特别决议"这些否定性结论

1. **多选题陷阱**：漏选 C 或多选 B/D 都不得分，必须四个选项全部判断准确
...
1. **表格后处理**：对解析出的表格做格式校验（行列数一致、数值列对齐），异常时回退到手动提取

1. **公式补标注**：在公式附近插入描述性文本（如"该公式表示：现金价值 = 累计保费 × 75%"），帮助 LLM 理解公式含义

1. **段落合并**：同一段落的文本可能被解析成多行，按标点和语义做合并

1. **文档结构标注**：为每个段落标注所属章节路径（如"第二章 > 第三节 > 现金价值计算"），增强检索时的上下文信息
...
1. VLM 对图像做 patch 编码（如 ColPali 产出每页 1024 个 patch embedding）

1. 查询时，问题文本和页面图像的 patch embedding 做 MaxSim 匹配（取每个 query token 与所有 patch 的最大相似度之和）
...
1. **局部性**：擅长回答"这段话说了什么"，但回答不了"这些文档整体讲了什么"（四份保险条款的退保规则有什么系统性差异？）

1. **无反思**：检索到什么就用什么，不评估质量，不修正方向
...
1. **知识图谱构建**：将抽取出的实体作为节点、关系作为边，构建一张有向/无向图。这张图不是简单的关键词共现网络，而是携带了语义信息的知识结构。

1. **社区检测**：用 Leiden 算法在图结构上发现紧密关联的实体群组（社区）。每个社区对应一个主题领域，比如"退保规则社区"可能包含"退保费用""退保费用率""现金价值""保单年度"等实体及其关系。

1. **社区摘要生成**：对每个检测出的社区，用 LLM 生成一个总结性的摘要，描述这个社区的核心信息。这一步是 GraphRAG 区别于传统图方法的关键：不是把原始文本片段直接存到图里，而是对每个主题生成高层摘要，使得检索时可以直接读取"关于退保规则的核心要点是什么"。

1. **两种检索模式**：

1. **Local Search（局部搜索）**：查询时匹配相关实体，从实体出发沿图的边扩散到邻居节点，获取与问题直接相关的实体和关系。适合事实查找类问题（"平安智盈金生的现金价值怎么算？"）。

1. **Global Search（全局搜索）**：查询时匹配相关社区，用社区的摘要生成部分回答，再将所有部分回答汇总成最终答案。适合全局理解类问题（"四份条款的退保规则有什么系统性差异？"）。这一步本质上是 Map-Reduce：社区摘要做 Map，LLM 汇总做 Reduce。
...
1. **关系向量索引**：每条关系边也有独立的向量表示（由关系描述和两端实体信息的 embedding 组成）。查询时，问题向量与关系向量做相似度匹配，直接找到相关的关系。这能捕捉到跨实体的语义模式（比如"退保费用率逐年递减"这种趋势性关系），而不仅仅是单个实体的匹配。

1. **关键词引导查询**：LightRAG 从用户问题中提取关键词，将这些关键词的向量拼接成组合查询向量。这种方式比直接用整个问题做向量检索更聚焦，减少了语义稀释。
...
1. **聚类**：用高斯混合模型（GMM）对叶子节点的 embedding 做软聚类，每个节点可以属于多个聚类

1. **摘要**：对每个聚类，用 LLM 对聚类内的所有文本块生成一个摘要。这个摘要成为上一层的一个节点

1. **递归**：对上层节点重复聚类 + 摘要，直到只剩一个根节点

1. **最终结构**：一棵多层树，底层是原始文本块（精确细节），中层是主题摘要（如"退保规则""身故保险金规则"），顶层是全局摘要（如"该产品的核心条款概述"）
...
1. 对每个片段做三方面评估：相关性（这个片段和问题相关吗？）、支持度（这个片段支持还是反对某个观点？）、有用性（这个片段对回答有帮助吗？）

1. 对低质量片段重新检索或修正

1. 生成最终答案时，标注每个论断的来源片段和可信度

**CRAG（Corrective RAG）**（2024 年 1 月发布, arXiv:2401.15884）[10] 在 Self-RAG 基础上更进一步，加入了自动纠错机制：

1. 检索文档片段

1. 用检索质量评估器对整体检索结果打分（0-1 分）

1. 根据分数分三级处理：

1. 分数 ≥ 0.7（CORRECT）：检索质量好，直接使用

1. 分数 0.3-0.7（AMBIGUOUS）：质量一般，保留当前结果同时用不同关键词补充检索，两轮结果合并

1. 分数 ≤ 0.3（INCORRECT）：质量差，丢弃检索结果，切换到全文阅读或其他策略

1. 最终生成前再做一次"幻觉检查"，过滤掉检索结果中不存在的论断
...
1. **Errors & Solutions**（遇到的错误和解决方案）

1. **Code Evolution**（代码最终版本，丢弃中间尝试）

1. **System Context**（项目结构、环境配置）

1. **Design Decisions**（架构决策及理由）
...
1. 从文档中提取出的公式、数值、条件判断（核心证据）

1. 检索到的原文片段中与题目直接相关的部分

1. 文档章节标题和结构信息（用于定位和引用）

1. 中间推理过程的摘要

1. 检索到的原文中经判断为不相关的部分（最低优先级，优先丢弃）
...
1. **从文档中提取的精确数值和公式**：如"现金价值 = 累计保费 × 75%"

1. **包含例外条件的条款原文**：如"但意外伤害导致的情形除外"
...
1. **神圣不可删改性**：凡是经过行动验证的有效配置、避坑指南、关键路径，在重构时严禁丢弃。可以压缩文字、可以迁移层级（从 L2 移到 L3），但不能丢失信息的准确性和可追溯性。

1. **禁止存储易变状态**：当前时间戳、临时 Session ID、运行中的 PID、某个具体绝对路径。这些随时间/会话高频变化的数据严禁写入记忆。

1. **最小充分指针**：上层只留能定位下层的最短标识，多一词即冗余。
...
1. **档案记忆（Archival Memory）**：文档、文章、知识库等外部资料。LLM 通过 `archival_memory_search` 函数做语义检索。这部分最接近本赛题的"文档检索"场景。

1. **工作记忆（Working Memory）**：当前推理状态。LLM 可以通过 `core_memory_append` 和 `core_memory_replace` 函数主动更新（比如"用户刚才说了喜欢 Python，我把这个记下来"）。
...
1. 读第 6-10 页 → 提取退保规则 → 追加到摘要

1. 读第 11-15 页 → 发现是模板化条款（释义、投保须知）→ 跳过，不消耗主上下文

1. ...继续，直到模型判断信息足够回答问题

1. 最终提交给 LLM 的是结构化摘要而非 50 页原文
...
1. **多文档对比阶段**：读完文档 A 后生成结构化摘要（如"产品 X：身故保险金 = max(已交保费, 现金价值)"），读文档 B 时只带着 A 的摘要而非原文，对比时用摘要即可

1. **验证阶段**：自检时不需要重新读全文，只需要带上第一轮推理的结构化摘要（公式 + 代入值 + 计算结果）做交叉验证
...
1. **从文档中提取的精确数值和公式**：保留原文，不压缩

1. **包含例外条件的条款原文**（如"但...除外""不适用...情形"）：保留原文
...
1. 第二轮：让模型验证自己的答案，检查每个选项的证据是否充分
...
1. **不能做数据整理**：跨文档对比需要先把多个文档的数据提取出来，再统一计算。Bash 执行的 Python 脚本可以自由定义数据结构
...
1. **变量传递**：中间结果存为 Python 变量，自动流入下一步，不需要 JSON 来回传

1. **条件逻辑**：可以用 `if/else` 做分支判断，比如"如果搜索结果不相关，换个关键词重搜"

1. **模型原生能力**：LLM 在代码生成上的训练数据远多于 JSON 工具调用格式，生成质量更稳定
...
1. **模型无关**：smolagents 支持百炼（OpenAI 兼容 API）和阶跃星辰，通过 LiteLLM 适配，环境变量切换即可

1. **安全执行**：Python 代码在受限的 `LocalPythonInterpreter` 中执行，限制 `import os`/`subprocess` 等危险模块

1. **Token 预算控制**：smolagents 本身不额外消耗 Token，但 Code Agent 的输出比 JSON Tool Calling 更紧凑
...
1. 模型输出一个固定长度的向量（比如 768 维或 1536 维），这串数字就是这段文本的"指纹"

1. 两段文本如果语义相近（比如"退保费用"和"退保手续费"），它们的向量指纹也会很接近
...
1. **精排（Rerank）**：把"问题 + 每个候选片段"一起送入 Cross-Encoder，模型输出一个相关性分数

1. **取 top K**：按分数排序，只把最高分的 5-10 个片段交给 LLM
...
1. **注意力衰减**：即使模型技术上支持 128K 上下文，实验表明模型对 prompt 中间部分的内容记忆效果会下降（称为"中间丢失"问题）。关键信息如果藏在 prompt 中段，模型可能"视而不见"。
...
1. **责任免除**：不保什么。通常用"因下列情形之一导致...的，本公司不承担保险责任"开头。这是条款定位题的高频考点。

1. **保险期间与等待期**：等待期内出险通常不赔或只退保费，但意外伤害导致的出险往往例外（"因意外伤害发生上述情形的，无等待期"）。

1. **保险金额与保费**：基本保额、累计保费、个人账户价值之间的关系。

1. **退保与现金价值**：退保能拿回多少钱，通常和已交保费、持有年限、账户收益挂钩。不同产品的现金价值计算公式差异很大。

1. **释义**：对条款中出现的专业术语（如"意外伤害""重大疾病""医院"）给出明确定义。
...
1. **具体义务**：金融机构需要做什么。比如"应当识别受益所有人""应当保存客户身份资料至少 5 年"

1. **时限要求**：很多条款有明确的时间节点（"在建立业务关系后 10 个工作日内""在获知变更信息后 30 日内"）

1. **例外条款**："但书"（"但是..."）后面的内容往往是考点。比如"应当识别受益所有人，但各级党的机关除外"

1. **罚则/法律责任**：违反规定会受到什么处罚
...
1. 关键科目：货币资金、应收账款、存货、固定资产、短期借款、长期借款、股东权益

1. **利润表（Income Statement）**

1. 反映一段时间内的经营成果

1. 基本等式：营业收入 - 营业成本 - 费用 = 净利润

1. 关键科目：营业总收入、营业成本、销售费用、管理费用、研发费用、营业利润、净利润、归属于母公司股东的净利润

1. **现金流量表（Cash Flow Statement）**

1. 反映一段时间内的现金流入和流出

1. 分三类：经营活动、投资活动、筹资活动

1. 关键指标：经营活动产生的现金流量净额（衡量企业"造血能力"的核心指标）
...
1. **评级信息**：主体评级、债项评级、评级展望

1. **增信措施**：担保、抵押、质押等信用增进安排

1. **特殊条款**：

1. 回售条款：投资者有权在特定时间按约定价格将债券卖回给发行人

1. 赎回条款：发行人有权在特定时间提前赎回债券

1. 调整票面利率条款：发行人有权调整后续计息期间的利率

1. 交叉违约条款：发行人在其他债务上违约，本债券也视为违约
...
1. **竞争格局**：主要参与者、市场份额、集中度（CR4、CR8）

1. **驱动因素**：政策、技术、需求、供给等影响行业发展的关键因素

1. **数据与预测**：历史数据、增长率、未来预测

1. **投资建议**：看好/看平/看空，风险提示
...
[21] Claude Code. Anthropic, 2025. [https://docs.anthropic.com/en/docs/claude-code](https://docs.anthropic.com/en/docs/claude-code)
...
[27] 阿里云百炼. Qwen-Plus 模型更新公告（2025年8月1日，上下文长度扩展至 1M）. [https://www.aliyun.com/notice/detail?notice-id=117430](https://www.aliyun.com/notice/detail?notice-id=117430)
